...
const calibration = () => {
  ...
  const realArray = convertFileToArray("src/Day1/day-1-data.txt");
...
console.log(calibration(), "Dis is the answer");

// --------------------------- Part 2 ---------------------------

const convertLettersToNumbers = (inputString: string) => {
  // create lookup table
...
const calibrationPart2 = () => {
  ...
  const processedTestInputPromise = testInputPart2.map(convertLettersToNumbers);

  const processedRealInputPromise = realArray.map(convertLettersToNumbers);
...
// O(N)
console.log(calibrationPart2(), "Dis is the answer");
